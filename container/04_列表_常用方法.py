...
print(
    f"{Fore.GREEN} =====1.使用 index 方法，尋找指定元素在列表中第一次出現的下標，傳回值是：元素下標。====="
)
fruits = ["香蕉", "蘋果", "橙子", "香蕉"]
...
print(
    f"{Fore.GREEN} =====2.使用 count 方法，統計某個元素在列表中出現的次數，傳回值是：元素出現的次數。 ====="
)
...
print(f"{Fore.GREEN} =====3.使用 reverse 方法，對列表進行反轉（會改變原列表）。=====")
...
print(
    f"{Fore.GREEN} =====4.1 若列表中的元素：都是數字，則按照數字的大小順序進行排序。 ====="
)
nums = [23, 11, 32, 30, 17]
...
print(f"{Fore.GREEN} =====4.2 若列表中的元素：既有數字，又有字串，那就會報錯。 =====")
...
print(
    f"{Fore.GREEN} =====4.3 若列表中的元素：都是字串，則按照字串的 Unicode 編碼大小進行排序。 ====="
)
msg_list = ["北京", "北硅谷", "北好"]
msg_list.sort()
print(msg_list)
